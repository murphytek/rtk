...
/// True only for the `.` / `..` self/parent entries.
///
...
mod tests {
    ...
    // Regression (murphytek #13): a real entry whose name ends in '.' (e.g. a file
    // "report." or a directory "build.") was misclassified as the "." / ".." dot-dir
    // entry by the loose `ends_with('.')` heuristic and silently dropped.
    #[test]
    fn test_compact_keeps_file_named_with_trailing_dot() {
        ...
        assert_eq!(
            parsed_count, 1,
            "the 'report.' file must be parsed, not dropped"
        );
        assert!(
    ...
    fn test_compact_keeps_dir_named_with_trailing_dot() {
        ...
        assert_eq!(
            parsed_count, 1,
            "the 'build.' dir must be parsed, not dropped"
        );
        assert!(
